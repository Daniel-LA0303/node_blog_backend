...
const getNotificationsByUserController = async (req: any, res: any, next: any) => {
    try {
            ...
            // mapping response
            res.status(200).json(
                new ApiResponse(200, "/api/notifications/:id" + req.path, req.method, "Success get notification by user paginated", result, false)
            );
...
const changeStatusController = async (req: any, res: any, next: any) => {
    try {
            ...
            // mapping response
            res.status(200).json(
                new ApiResponse(200, "/api/notifications/:id" + req.path, req.method, "Success change status notifiction", result, false)
            );
